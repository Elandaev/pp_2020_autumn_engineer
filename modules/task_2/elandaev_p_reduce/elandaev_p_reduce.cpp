...
int Reduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype type, MPI_Op op, int root, MPI_Comm comm) {
    ...
    if (type == MPI_INT) {
        ...
        std::memcpy(buffer, sendbuf, count * sizeof(int));
    } else if (type == MPI_FLOAT) {
        buffer = new float[count];
        std::memcpy(buffer, sendbuf, count * sizeof(float));
    } else if (type == MPI_DOUBLE) {
        buffer = new double[count];
        std::memcpy(buffer, sendbuf, count * sizeof(double));
    } else {
        return MPI_ERR_TYPE;
    }
    
    if(rank != root){
        MPI_Send(buffer, count, type, root, 0, comm);
    }
    ...
    if(rank == root){
        ...
        for(int i = 0; i < size; i++){
            if(i != root){
                MPI_Recv(buffer, count, type, i, 0, comm, &status);
            }
            if (type == MPI_INT) {
                if (op == MPI_MAX) {
