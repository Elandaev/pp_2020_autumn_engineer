...
TEST(Parallel_Operations_MPI, IntMax) {
    ...
    Reduce(&sendbuf, &recvbuf, count, MPI_INT, MPI_MAX, root, MPI_COMM_WORLD);
...
TEST(Parallel_Operations_MPI, IntMin) {
    ...
    int sendbuf = rank + 1;
    int recvbuf = 10000000;
    const int count = 1;
    const int root = 0;
    int min = 1;
    
    MPI_Reduce(&sendbuf, &min, count, MPI_INT, MPI_MIN, root, MPI_COMM_WORLD);

    Reduce(&sendbuf, &recvbuf, count, MPI_INT, MPI_MIN, root, MPI_COMM_WORLD);
    if (rank == root)
...
TEST(Parallel_Operations_MPI, IntSum) {
    ...
    Reduce(&sendbuf, &recvbuf, count, MPI_INT, MPI_SUM, root, MPI_COMM_WORLD);
    if (rank == root) {
...
TEST(Parallel_Operations_MPI, IntProd) {
    ...
    Reduce(&sendbuf, &recvbuf, count, MPI_INT, MPI_PROD, root, MPI_COMM_WORLD);
    if (rank == root) {
...
TEST(Parallel_Operations_MPI, FloatMax) {
    ...
    Reduce(&sendbuf, &recvbuf, count, MPI_FLOAT, MPI_MAX, root, MPI_COMM_WORLD);
...
TEST(Parallel_Operations_MPI, FloatMin) {
    ...
    float sendbuf = static_cast<float>(rank + 1);
    float recvbuf = 1000000000000.0f;
    const int count = 1;
    const int root = 0;
    const float min = static_cast<float>(1);

    Reduce(&sendbuf, &recvbuf, count, MPI_FLOAT, MPI_MIN, root, MPI_COMM_WORLD);
...
TEST(Parallel_Operations_MPI, FloatSum) {
    ...
    Reduce(&sendbuf, &recvbuf, count, MPI_FLOAT, MPI_SUM, root, MPI_COMM_WORLD);
...
TEST(Parallel_Operations_MPI, FloatProd) {
    ...
    Reduce(&sendbuf, &recvbuf, count, MPI_FLOAT, MPI_PROD, root, MPI_COMM_WORLD);
...
TEST(Parallel_Operations_MPI, DoubleMax) {
    ...
    Reduce(&sendbuf, &recvbuf, count, MPI_DOUBLE, MPI_MAX, root, MPI_COMM_WORLD);
...
TEST(Parallel_Operations_MPI, DoubleMin) {
    ...
    double sendbuf = static_cast<double>(rank + 1);
    double recvbuf = 100000.0;
    const int count = 1;
    const int root = 0;
    const double min = static_cast<double>(1);

    Reduce(&sendbuf, &recvbuf, count, MPI_DOUBLE, MPI_MIN, root, MPI_COMM_WORLD);
...
TEST(Parallel_Operations_MPI, DoubleSum) {
    ...
    Reduce(&sendbuf, &recvbuf, count, MPI_DOUBLE, MPI_SUM, root, MPI_COMM_WORLD);
...
TEST(Parallel_Operations_MPI, DoubleProd) {
    ...
    Reduce(sendbuf, &recvbuf, count, MPI_DOUBLE, MPI_PROD, root, MPI_COMM_WORLD);
...
TEST(Parallel_Operations_MPI, IntArrMax) {
    ...
    MPI_Reduce(sendbuf, example, count, MPI_INT, MPI_MAX, root, MPI_COMM_WORLD);
...
TEST(Parallel_Operations_MPI, FloatArrSum) {
    ...
    for (int i = 0; i < count; i++){
        ...
        recvbuf[i] = 0.0f;
        example[i] = sendbuf[i] * size;
    }
    
    Reduce(sendbuf, recvbuf, count, MPI_FLOAT, MPI_SUM, root, MPI_COMM_WORLD);
    
    MPI_Reduce(sendbuf, example, count, MPI_FLOAT, MPI_SUM, root, MPI_COMM_WORLD);
...
TEST(Parallel_Operations_MPI, DoubleArrProd) {
    ...
    MPI_Reduce(sendbuf, example, count, MPI_DOUBLE, MPI_PROD, root, MPI_COMM_WORLD);
